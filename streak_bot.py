...
try:
    masked = "<empty>" if not RIOT_API_KEY else (RIOT_API_KEY[:6] + "..." + RIOT_API_KEY[-4:])
except Exception:
    masked = "<error>"
logger.info("RIOT_API_KEY present: %s", masked)
...
async def riot_poll_loop(state: BotState, templates):
    ...
    headers = {"X-Riot-Token": RIOT_API_KEY, "User-Agent": "streak_bot/1.0"}
    ...
    while summoner is None:
        try:
            ...
            masked = "<error>"
        logger.error(
            "No se pudo cargar el invocador de Riot (verifica REGION, SUMMONER_NAME y RIOT_API_KEY). RIOT_API_KEY present=%s. Reintentando en 60s",
            masked,
        )
